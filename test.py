...
def train_one_epoch(train_loader, model, optimizer):
    ...
    for batch_i, (norms, coords, labels, g_mat, idxs) in enumerate(train_loader):
        ...
        data_dict = {'batch': batches.cuda(), 'feat': norms.flatten(end_dim=1).cuda().to(torch.float32), 'coord': coords.flatten(end_dim=1)[:,0:3].cuda().to(torch.float32), 'labels': labels.flatten().cuda(), 'grid_size': torch.tensor(0.001).to(torch.float32)}
