...
if __name__ == "__main__":
    ...
    data_dict = {'batch': batches.cuda(), 'feat': norms.flatten(end_dim=1).cuda().to(torch.float64), 'coord': coords.flatten(end_dim=1)[:,0:3].cuda().to(torch.float32), 'labels': labels.flatten().cuda().to(torch.float64), 'grid_size': 0.001}
