...
def train_one_epoch(train_loader, model, optimizer):
    ...
    for batch_i, (norms, coords, labels, g_mat, idxs) in enumerate(train_loader):
        ...
        loss_seg = F.cross_entropy(results['feat'].reshape(labels.shape[0],-1, labels.shape[1]).permute(0,2,1), labels.cuda(), weight=train_loader.dataset.segweights.cuda())
        ...
        iou_list.append(cal_IoU_Acc_batch(results['feat'].reshape(labels.shape[0],-1, labels.shape[1]).permute(0,2,1), labels.cuda()))
    ...
    for i in range(args.num_votes):
        ...
        with torch.no_grad():
            for batch_idx, (norms, coords, labels, g_mat, idxs) in enumerate(val_loader):
                ...
                # pts, gts, egts, eweights, gmatrix = pts.cuda(), gts.cuda(), egts.cuda(), eweights.mean(dim=0).cuda(), gmatrix.cuda()
                # seg_preds, seg_refine_preds, seg_embed, edge_preds = model(pts, gmatrix, idxs)
                loss_seg = F.cross_entropy(results['feat'].reshape(labels.shape[0],-1, labels.shape[1]).permute(0,2,1), labels.cuda(), weight=val_loader.dataset.segweights.cuda())
                ...
                # loss_edge_avg += loss_edge.item()
                # loss_contra_avg += loss_contra.item()
                iou_avg.append(cal_IoU_Acc_batch(results['feat'].reshape(labels.shape[0],-1, labels.shape[1]).permute(0,2,1), labels.cuda().to(torch.float32)))
